...
const H_COLOR: Record<string, string> = {
  in1: '#f97316', in2: '#14b8a6', in3: '#ec4899',
  out1: '#6366f1', out2: '#f59e0b',
}
...
function ColoredEdgeLayer() {
  ...
  return (
    <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none', overflow: 'visible' }}>
      {edges.map((e: Edge) => {
        const color = H_COLOR[e.source.handleId] ?? H_COLOR[e.target.handleId] ?? '#818cf8'
        return (
          <path key={e.id} d={bezierPath(e.source.pt, e.target.pt)}
...
function RouterNode({ defaultPosition }: { defaultPosition: { x: number; y: number } }) {
  ...
  const dot = (
    ...
    <div
      ...
      style={{
        position: 'absolute', [side]: -8, top, transform: 'translateY(-50%)',
        width: 16, height: 16, borderRadius: '50%',
        background: H_COLOR[id] ?? T.accent, border: `2px solid ${T.bg}`, cursor: 'crosshair',
      }}
    />
  )
  ...
  return (
    <div ref={nodeRef} style={{
      position: 'absolute', top: 0, left: 0, width: 170, height: 150,
      ...
      <div style={{ padding: '7px 14px', borderBottom: `1px solid ${T.border}`, fontSize: 11, color: T.muted, letterSpacing: 1 }}>
        ROUTER
      </div>
      {dot(in1Ref, 'in1', 'left', '33%', 'target')}
      {dot(in2Ref, 'in2', 'left', '57%', 'target')}
      {dot(in3Ref, 'in3', 'left', '81%', 'target')}
      <div style={{ position: 'absolute', left: 14, top: '33%', transform: 'translateY(-50%)', fontSize: 11, color: H_COLOR.in1 }}>in1 <span style={{ color: T.muted }}>×∞</span></div>
      <div style={{ position: 'absolute', left: 14, top: '57%', transform: 'translateY(-50%)', fontSize: 11, color: H_COLOR.in2 }}>in2 <span style={{ color: T.muted }}>×1</span></div>
      <div style={{ position: 'absolute', left: 14, top: '81%', transform: 'translateY(-50%)', fontSize: 11, color: H_COLOR.in3 }}>in3 <span style={{ color: T.muted }}>×∞</span></div>
      {dot(out1Ref, 'out1', 'right', '40%', 'source')}
      {dot(out2Ref, 'out2', 'right', '72%', 'source')}
      <div style={{ position: 'absolute', right: 14, top: '40%', transform: 'translateY(-50%)', fontSize: 11, color: H_COLOR.out1 }}>out1</div>
      <div style={{ position: 'absolute', right: 14, top: '72%', transform: 'translateY(-50%)', fontSize: 11, color: H_COLOR.out2 }}>out2</div>
    </div>
  )
}
    ...
    const onEdge = ({ edge }: { edge: Edge }) => {
      ...
      if (nodeId === 'router' && SINGLE_HANDLES.has(handleId)) {
        const duplicate = engine.getEdges().some(
          (e) => e.id !== edge.id && e.target.nodeId === nodeId && e.target.handleId === handleId,
        )
        if (duplicate) { engine.removeEdge(edge.id); flash(`"${handleId}" only accepts 1 connection`) }
      }
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={toolbar}>
        <span style={{ fontSize: 11, color: T.muted }}>
          ...
          <span style={{ color: H_COLOR.in2 }}>in2</span> accepts exactly 1
        </span>
      </div>
      <div style={{ flex: 1, position: 'relative' }}>
        <FlowCanvas canvasRef={canvasRef}>
          {LEFT_NODES.map((n) => <SimpleNode key={n.id} id={n.id} label={n.label} kind={n.kind} defaultPosition={{ x: n.x, y: n.y }} />)}
          <RouterNode defaultPosition={{ x: 250, y: 130 }} />
          {RIGHT_NODES.map((n) => <SimpleNode key={n.id} id={n.id} label={n.label} kind={n.kind} defaultPosition={{ x: n.x, y: n.y }} />)}
          <ColoredEdgeLayer />
        </FlowCanvas>
        {toast && (
          <div style={{
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          '**Why this scenario**: Real workflow nodes expose multiple typed ports at different positions.',
          '',
          '**APIs used**: `useHandle(nodeId, handleId, type)` × 5 handles · `useDraftEdge()` for draft edge · `engine.getEdges()` + `engine.removeEdge(id)` for capacity enforcement',
          '',
          '**Try this**: 1) Connect Source A and Source B both to Router in1 — both succeed (×∞). 2) Connect Source A to in2, then try Source B to in2 — the second is rejected.',
        ].join('\n'),
      },
    },
  },
