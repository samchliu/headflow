...
function TypedNode({ id, label, kind, defaultPosition }: {
  id: string; label: string; kind: string; defaultPosition: { x: number; y: number }
}) {
  ...
  return (
    <div ref={nodeRef} style={{
      ...
      cursor: 'grab', userSelect: 'none', color: T.text,
      ...
      />
      <div style={{ fontSize: 10, color, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}>{kind}</div>
      {label}
...
function Inner({ canvasRef, allowSelf, onToggleSelf }: {
  canvasRef: (el: HTMLElement | null) => void; allowSelf: boolean; onToggleSelf: () => void
}) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <FlowCanvas canvasRef={canvasRef}>
          {NODES.map((n) => (
            <TypedNode key={n.id} id={n.id} label={n.label} kind={n.kind} defaultPosition={{ x: n.x, y: n.y }} />
          ))}
          <EdgeLayer />
        </FlowCanvas>
        {toast && (
...
function ConnectionRulesLabStory() {
  ...
  const toggle = () => { setAllowSelf((v) => !v); setKey((k) => k + 1) }
    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**Try this**: 1) Connect two same-type nodes — succeeds. 2) Try Event→Data — red toast appears and the edge is removed. 3) Toggle "Self-loop: Blocked/Allowed" then drag a handle back to its own node.',
        ].join('\n'),
      },
    },
  },
